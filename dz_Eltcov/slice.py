...
def shift(s):
    ...
    return ' '.join(list(s.split(' '))[-1::len(s)]) + ' ' + ' '.join(list(s.split(' '))[0:-1:1])
...
def inside(s):
    st = list(s.split(' ')[::])
    return ' '.join(st[1:len(st)/2:] + st[0::-1] + st[-1::1] + st[len(st)/2:len(st)-1:])
